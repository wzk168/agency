...
<script type="text/javascript">
	...
	function upBranchCompany() {
		...
		if(bValid)
		{
			$.ajax({
				type : "post",
				url : "upBrabchCompany.html",
				data : $("#upbranchfrom").serialize(),
				dataType : "text",
			...
			<tbody>
				<tr>
					...
					<select name="bcgInscompanyId" 
						id="bcgInscompanyIdId" class="dfinput">
							...
							<option value="${insuran.insuranceId}" <c:if test="${insuran.insuranceId eq branch.bcgInscompanyId }">selected="selected"</c:if>>${insuran.insuranceName}</option>
							</c:forEach>
				...
				<tr>
					...
					<td colspan="3"><input type="text" name="bcgBranchName"
						id="bcgBranchNameId"  class="dfinput" size="60" value="${branch.bcgBranchName}"
						/><i class="bcgBranchNamespan"></i></td>
				</tr>
				<tr>
					<th align="right" width="10%">公司简称</th>
					<td width="30%"><input type="text" name="bcgBranchShort"
						id="bcgBranchShortId" value="${branch.bcgBranchShort}" 
					...
					<td width="31%"><input type="text" name="bcgBranchMan"
						id="bcgBranchManId" class="dfinput" size="30" value="${branch.bcgBranchMan}"
				...
				<tr>
					...
					<td><select name="bcgBranchSituation"  
						id="bcgBranchSituationId" class="dfinput">
							...
							<option value="1" <c:if test="${branch.bcgBranchSituation==1}">selected="selected"</c:if>>正常出单</option>
							<option value="2" <c:if test="${branch.bcgBranchSituation==2}">selected="selected"</c:if>>停止出单</option>
					...
					<td><select id="bcgBranchTypeId" name="bcgBranchType" class="dfinput">
							...
							<option value="1" <c:if test="${branch.bcgBranchType==1}">selected="selected"</c:if>>综合业务</option>
							<option value="2" <c:if test="${branch.bcgBranchType==2}">selected="selected"</c:if>>产险</option>
							<option value="3" <c:if test="${branch.bcgBranchType==3}">selected="selected"</c:if>>车险</option>
							<option value="4" <c:if test="${branch.bcgBranchType==4}">selected="selected"</c:if>>寿险</option>
					</select><i class="bcgBranchTypespan"></i></td>
				</tr>
				<tr>
					<th>公司电话</th>
					<td><input type="text" name="bcgBranchPhone" value="${branch.bcgBranchPhone}"
						id="bcgBranchPhoneId"  class="dfinput" size="30" />
						<i class="bcgBranchPhonespan"></i>
						</td>
					<th>传真</th>
					<td><input type="text" name="bcgBranchFax" value="${branch.bcgBranchFax}"
				...
				<tr>
					...
					<td colspan="3"><input type="text" name="bcgBranchRemark"
						...
						value="${branch.bcgBranchRemark}"/><i class="bcgBranchRemarkspan"></i></td>
				</tr>
				<tr align="center"><td colspan="4">
				<input type="hidden" name="bcgBranchId" value="${branch.bcgBranchId}">
				<input type="hidden" name="bcgCompanyId" value="${branch.bcgCompanyId}">
				<input type="button" value="修改保存" onclick="upBranchCompany()" class="ibtn">
