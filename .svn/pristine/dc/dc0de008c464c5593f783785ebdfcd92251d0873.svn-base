...
<script type="text/javascript">
		...
		//加载添加使用公司界面
		function beforaddUserCompany() {
			window.location.href = "beforaddUserCompany.html";	
		}
		//查询信息
		function userCompanyList() {
				window.location.href = "userCompanyList.html?ucsCompanyName="+$("#ucsCompanyNameId").val()+"&pageNow=1";
		}
		//进入修改使用公司界面
		function beforUpUserCompany() {
			var val = $("input[name='ucsCompanyId']:checked").val();
			if(typeof(val)=='undefined'){
            	alert("请选择使用公司再点击修改按钮");
                    }else
                    {
                    	window.location.href = "beforUpUserCompany.html?ucsCompanyId="+$("input[name='ucsCompanyId']:checked").val();	
