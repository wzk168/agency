...
public class TeamInfoDaoImpl extends BaseDaoImpl implements TeamInfoDao 
{
	...
	public int addTeamInfo(TeamInfo teamInfo) {
		String addTeamSql="INSERT INTO ay_teaminfo_msg (ts_team_id,ts_team_name,ts_team_principal,ts_dept_id,ts_team_type,ts_company_id) VALUES(?,?,?,?,?,?)";
		try {
			return this.getJdbcTemplate().update(addTeamSql, teamInfo.getTsTeamId(),
					teamInfo.getTsTeamName(),teamInfo.getTsTeamPrincipal(),
					teamInfo.getTsDeptId(),teamInfo.getTsTeamType(),teamInfo.getTsCompanyId());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
	...
	public List<TeamInfo> loadTeamList(String teamName, int teamType,
		...
		Object[] pares=null;
		StringBuffer strbuf=new StringBuffer("SELECT ts_team_id,ts_team_name,ts_team_principal,ts_dept_id,ts_team_type,ts_company_id FROM ay_teaminfo_msg WHERE 1=1 ");
		...
		try {
			List<TeamInfo> teamlist=this.getJdbcTemplate().query(strbuf.toString(),pares,new TeamInfoSet());
	...
	public int updateTeamInfo(TeamInfo teamInfo) {
		String selTeamSql="UPDATE ay_teaminfo_msg SET ts_team_name=?,ts_team_principal=?,"
				+ "ts_dept_id=?,ts_team_type=?,ts_company_id=? WHERE ts_team_id=?";
		try {
			return this.getJdbcTemplate().update(selTeamSql,
					teamInfo.getTsTeamName(),teamInfo.getTsTeamPrincipal(),
					teamInfo.getTsDeptId(),teamInfo.getTsTeamType(),teamInfo.getTsCompanyId(),teamInfo.getTsTeamId());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
